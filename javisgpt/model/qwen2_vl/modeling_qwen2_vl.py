...
class Qwen2VLRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids):
        ...
        # So we expand the inv_freq to shape (3, bs, dims//2, 1)
        ...
        with torch.autocast(device_type=device_type, enabled=False):
            freqs = (inv_freq_expanded.float() @ position_ids_expanded.float()).transpose(2, 3) # shape (3, bs, positions, dims//2)
            emb = torch.cat((freqs, freqs), dim=-1)  # shape (3, bs, positions, dims)
            cos = emb.cos()
            sin = emb.sin()
...
def rotate_half(x):  # TODO: why not [..., ::2] ?
...
def apply_multimodal_rotary_pos_emb(q, k, cos, sin, mrope_section, unsqueeze_dim=1):
    ...
    """
    mrope_section = mrope_section * 2  # t_sec, h_sec, w_sec

    # cos,sin : shape(3, bs, seq_len, head_dim) -> shape(bs, 1, seq_len, head_dim)
    ...
    # q,k: shape(bs, num_head, seq_len, head_dim)
...
class Qwen2VLModel(Qwen2VLPreTrainedModel):
    ...
    def forward(
        ...
        # the hard coded `3` is for temporal, height and width. shape[3, B, L]
