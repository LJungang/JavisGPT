...
from accelerate.utils import InitProcessGroupKwargs, GradientAccumulationPlugin, DistributedDataParallelKwargs
...
class LLaVATrainer(Trainer):
    ...
    def create_accelerator_and_postprocess(self):
        ...
        # TODO: `ddp_find_unused_parameters` is not helpful
        ddp_kwargs = DistributedDataParallelKwargs(find_unused_parameters=self.args.ddp_find_unused_parameters)
        ...
        self.accelerator = Accelerator(
            ...
            kwargs_handlers=[accelerator_kwargs] + ([ddp_kwargs] if self.args.ddp_find_unused_parameters else [])
        )
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        adapter_name, keys_to_match = get_adapter_keys_to_match_by_stage(self.args)
        if adapter_name is not None or getattr(self.args, "tune_mm_mlp_adapter", False) or (
            ...
            if self.args.local_rank == 0 or self.args.local_rank == -1:
                ...
                adapter_name = adapter_name or "mm_projector"
                torch.save(weight_to_save, os.path.join(output_dir, f"{adapter_name}.bin"))
